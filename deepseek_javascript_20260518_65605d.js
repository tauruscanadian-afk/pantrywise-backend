  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'usd',
          product_data: { name: 'PantryWise Recipe Pack' },
          unit_amount: 299,
        },
        quantity: 1,
      }],
...
app.post('/api/recipes', async (req, res) => {
  ...
  const prompt = `Using: ${ingredients.join(', ')}. Suggest 5 family recipes. Return JSON array: [{ name, cookTime, kidRating (1-5), missingItems, leftoverTip }]. No extra text.`;
  try {
    const response = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
